...
│   ├── plasticScm.ts        — SCM provider, status refresh, viewAllChanges 커맨드
│   ├── plasticCli.ts        — cm CLI wrapper, 캐시, 세마포어, in-flight coalesce
...
plasticScm.ts (SCM provider + 커맨드)
  └─ doRefresh — status 기반 변경 목록 갱신
  └─ viewAllChanges — Multi Diff 오픈
...
extension은 `Plastic SCM Diff` OutputChannel에 **timestamp prefix + 카테고리 prefix** (`[cm]`, `[cache]`, `[refresh]`, `[scm]`, `[provider]`, `[activate]`) 로 주요 단계를 기록한다.
...
- refresh 중 대량 `cm cat`을 추가하지 말 것. base content는 열린 diff 한 파일 단위로 lazy fetch한다.

### `plasticScm.ts`

- **`refreshInflight` 직접 await 남발 금지**. viewAllChanges는 현재 snapshot을 사용하고, 필요할 때만 초기 refresh를 기다린다.
